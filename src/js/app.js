...
function paginaAnterior() {
    ...
    paginaAnterior.addEventListener('click', function (e) {
        e.preventDefault(); // Agrega esta línea para prevenir el comportamiento predeterminado
...
function seleccionarServicio(servicio) {
    ...
    //comprobar si un servicio ya fue agregarlo o quitarlo
    if (servicios.some(agregado => agregado.id === id)) {
        //eliminarlo
        cita.servicios = servicios.filter(agregado => agregado.id !== id)
        divServicio.classList.remove('seleccionado') // Corregir aquí
    }
    else {
...
function mostrarResumen() {
    ...
    //Limpiar el contendio del resumen
    ...
    if (Object.values(cita).includes('') || cita.servicios.length === 0) {
        mostrarAlerta('Faltan datos de servicios, fecha u hoja', 'error', '.contenido-resumen', false)
